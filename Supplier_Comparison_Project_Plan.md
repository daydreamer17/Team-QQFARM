> **当前范围（2026-09-24）**：最小版本支持在制度检查页录入结构化供应商准入记录和 RoHS 证书，并结合 RAG 制度引用输出检查结论；证书真实性验证、外部 APL/注册表集成和金额审批记录仍不实现。

# Supplier Comparison 项目方案汇总

> **帮助 SME 采购人员把多家供应商报价转化为有来源、可核验、可审批的采购建议；当报价或需求变化时，重新比较，而不是沿用过期结论。**

| 项目 | 内容 |
| --- | --- |
| 参赛项目 | NUS-ISS × AWS — Show Me Your Agents |
| 已选题目 | **Supplier Comparison（供应商比选）** |
| 暂定产品名称 | **QuoteWise — Evidence-Grounded Supplier Comparison Agent** |
| 产品定位 | 辅助采购决策，不自主下单或付款 |
| 计划周期 | 2026-09-07 至 2026-09-25 开发；2026-09-28 提交 |
| 文档版本 | v0.7 · 2026-09-15 · pgvector 混合检索选型冻结 |
| 首版商品 | Electronics／Microcontroller MCU-9，同制造商、同料号、同封装及版本，全新且不允许替代 |
| 技术选型 | FastAPI＋Pydantic＋React＋LangGraph＋PostgreSQL／pgvector＋pdfplumber；制度使用 BM25＋embedding／rerank 检索，供应商合规事实使用结构化查询 |
| 部署与模型 | 官方 Lightsail＋Docker Compose／持久化卷；本地开发 API 跑通后切换主办方 API 测试 |

**阅读说明**：本文汇总选题、三周计划、交付物及 Delivery, Measurement and Controls。商品类别和工程选型已确定，具体模型接口、实例参数、人力投入与评测目标仍需核验；功能是拟实施设计，不表示已经完成。正式比赛日期、框架要求和提交格式以主办方通知为准。

本次修订将制度 RAG 与供应商合规门禁纳入正式 MVP，保留 MCU-9、LangGraph、PostgreSQL 及两阶段 API 路线。第一周建立版本／检查点骨架和最小部署，第二周完成补问、更新、制度检索、批准供应商／RoHS 精确查询、合规矩阵、审批和报告，最终周评测与提交。

**背景**: Procurement executives regularly receive quotations from multiple suppliers for the same products. Before every purchase, they manually compare prices, delivery lead times, payment terms, and supplier performance using spreadsheets and emails. As the number of suppliers and products grows, the comparison process becomes increasingly difficult, resulting in slower purchasing decisions and missed opportunities to negotiate better terms.

---

## 1. 为什么选择 Supplier Comparison

在已讨论的方向中，Supplier Comparison 比较适合在有限时间内同时展示业务价值和 Agent 工程能力。

| 选择理由 | 对本项目的意义 |
| --- | --- |
| 业务问题明确 | 直接帮助采购人员整理报价、检查条件并选择供应商 |
| 容易形成完整流程 | 从上传报价到补问信息、推荐、审批，都可以在一个应用中演示 |
| 结果可以验证 | 单位换算、总成本、交期、起订量和引用依据可以逐项核对 |
| 能展示动态处理 | 报价更新后重新计算、使旧审批失效，体现有状态工作流 |
| 与团队已有经验衔接 | 文档处理、工具调用、状态管理、前端展示和评测经验可以迁移 |

Quotation Preparation 和 Technician Scheduling 是此前比较过的备选方向，但本项目不同时实现客户报价生成、技师排程或完整采购系统。**先完成一条清晰的供应商比选主流程。**

## 2. 要解决的问题

### 2.1 目标用户与当前流程

首版目标用户是中小型电子制造企业的采购人员、采购主管，或兼任采购工作的业务负责人，使用同一明确规格 MCU 的多供应商报价验证通用比选流程。

根据用户提供的选题截图，采购人员会收到同一类商品的多家供应商报价，并通过邮件和电子表格手动比较价格、交期、付款条件及供应商表现。随着供应商和商品数量增加，比较更困难，采购决策也更慢。

当前流程可以概括为：

```text
收到多家报价 → 人工阅读 → 复制到表格 → 手动对齐和计算
→ 追问缺失信息 → 选择供应商 → 报价更新后再次人工修改
```

### 2.2 本项目优先解决的痛点

| 痛点 | 项目的解决方式 |
| --- | --- |
| 报价格式、商品描述和包装单位不同 | 提取关键字段，对齐明确可比较的规格与单位；不能确认时请求核实 |
| 表面单价不等于实际采购成本 | 通过代码计算起订量、整包装采购和已确认费用的影响 |
| 交期、付款条件、有效期等容易遗漏 | 把条件并列展示，并按用户明确要求检查 |
| 缺失信息容易被忽略或误填 | 显式标记缺失项，必要时暂停并请求补充，不猜测关键条款 |
| 推荐缺乏依据，难以复核 | 展示比较结果、推荐理由和对应的原始来源 |
| 报价或需求变化后旧结论仍被使用 | 跟踪版本，重新比较，并要求对新方案重新审批 |

### 2.3 项目目标与非目标

**目标：**减少整理和核对报价的重复工作，帮助用户形成可解释、满足采购约束的选择。

**非目标：**替代采购人员承担全部判断，自动议价、联系供应商、签约、下单或付款。缺少供应商表现数据时，也不生成没有依据的信誉评分。

## 3. Proposed Solution：整体方案

### 3.1 一句话方案

用户提交采购需求，通常上传 3–5 家供应商的报价，系统提取和核验信息、调用工具比较采购条件、补问缺失项，形成有依据的推荐，并通过人工审批生成最终决策报告。3–5 家是演示和评测的典型规模，不是必须凑齐的准入数量。

### 3.2 核心流程

```text
输入采购需求 + 上传供应商报价
                ↓
提取关键条款，并保留来源
                ↓
对齐商品规格、包装单位和数量
                ↓
检查缺失或冲突信息 → 必要时请求人工补充或纠正
                ↓
调用工具计算成本、检查采购约束
                ↓
比较可行方案，解释推荐理由与取舍
                ↓
人工审阅和批准 → 保存并导出决策报告
                ↓
报价或需求更新 → 旧结果及审批失效 → 重新比较与审批
```

### 3.3 决策原则

**先检查硬约束，再比较用户偏好。**

硬约束包括用户明确要求的规格、数量、预算和交期等。对于满足条件的供应商，再按用户指定的成本优先、交期优先等规则比较。付款条件可以并列展示，也可以在用户明确提出要求后作为约束。

对每家供应商分别判断“可行”“不可行”“待确认”。已有明确硬约束失败时，可以直接排除，不必补齐其余不影响排除结论的字段。例如交期已确定超限的 A 缺少运费，不应阻塞信息完整的 B、C。未知值不能被当作零，也不能被当作不合格。

只要仍有可能影响选择的待确认报价，就应展示草稿并请求补充，不能宣称“无可行方案”或给出最终最优结论。用户可明确排除某份报价，排除原因、操作者和版本必须留痕，报告说明比较范围。只有剩余范围内的判断已确定且没有可行供应商时，才输出“无可行方案”；全部报价被用户排除时，应请求重新选择范围。

**LLM 负责理解、追问和解释；代码工具负责计算、约束检查、状态和审批控制。** 不让模型随意决定评分权重，也不让模型自行放宽预算或交期。

### 3.4 Agent / Workflow Roles

以下职责通过一个 LangGraph 主图、独立业务模块和人工审批接口实现，不增加多 Agent 系统。图负责解析、理解、校验／补问、比较和解释，最终审批由 FastAPI 后端事务处理。

| 角色 | 职责 | 输入 | 输出 | 转人工的情况 |
| --- | --- | --- | --- | --- |
| 报价处理 | 提取、标准化并关联原始来源 | 报价和可用参考资料 | 结构化报价、待核实事项 | 信息缺失、歧义、规格冲突或格式不支持 |
| 比较与推荐 | 调用工具检查条件，比较可行方案并解释取舍 | 已核验报价、采购要求 | 比较表、推荐及理由 | 无可行方案或重要条款未确认 |
| 人工审核与审批 | 确认纠正、处理例外并批准方案 | 推荐、来源和未解决事项 | 批准记录或修改要求 | 问题未解决或超出审核人的权限 |

### 3.5 Agent 的可展示价值

主 Agent 在受限工具和后端状态规则内，完成“发现影响决策的歧义 → 读取相关证据 → 提出具体问题 → 根据回答继续调用比较工具 → 解释变化”的闭环。固定解析和算术步骤不需要由模型反复规划。

例如 B 写明“delivery 3 days”但未说明起算点，Agent 应结合原文询问是下单后还是付款后到货；若写的是发货时间，则请求确认到货时间。用户回答形成待校验的结构化修改，经确认后进入新版本。程序决定问题是否阻塞及何时可以审批，Agent 不能绕过这些规则。

界面展示简洁的操作记录：问题、证据引用、工具名称、执行状态、结果变化和下一步。展示可核验的动作与依据，不以模型内部推理文本作为证据。

## 4. MVP 范围与明确边界

| 维度 | 首版范围 | 暂不纳入 |
| --- | --- | --- |
| 业务场景 | Electronics／MCU-9，固定制造商、料号、封装及必要版本，全新且不允许替代 | 跨行业采购平台及器件兼容性自动判断 |
| 比较单位 | 每次一种明确规格的商品，典型规模 3–5 家供应商；排除后可仅剩一家可行 | 多商品组合采购和跨供应商拆单优化 |
| 输入格式 | 团队基于合成 CSV 生成英文文本 PDF／模板 CSV，pdfplumber 解析；每份 PDF 上限初值 5 页／5 MB、每任务 5 份，允许人工录入 | 扫描件、手写报价、任意复杂 Excel |
| 金额口径 | SGD；在已确认的一致税费口径下比较 | 自动汇率换算和复杂税务处理 |
| 主要条件 | 规格、数量、包装单位、起订量、成本、交期、付款条件和有效期 | 缺少数据时预测供应商表现 |
| 计价与履约 | 固定单价、明确计价单位、固定包装、按数量计 MOQ、一次交付、明确费用 | 阶梯价格、最低订单金额、分批交付及复杂折扣；遇到时请求适用条件或明确排除 |
| 排序 | 用户明确选择成本优先或交期优先；可确认第二排序项，否则并列 | 模型自定权重、未经确认的综合评分 |
| 信息补充 | 用户在应用内回答问题或更新资料 | 自动接入邮箱或 WhatsApp 并联系供应商 |
| 输出 | 比较表、有依据的推荐、版本与审批记录、可导出的报告 | 自动签约、下单和付款 |
| 制度 RAG 与合规门禁 | 3–5 份版本化演示制度使用 BM25＋pgvector＋embedding／rerank API 检索；批准供应商和 RoHS 使用 PostgreSQL 精确查询，确定性规则控制推荐资格 | ISO、框架合同、通用知识库、独立向量数据库、本地模型部署、规格书兼容性判断、任意跨语言问答 |
| 系统接入 | LangGraph＋本地开发 API／主办方 API，官方 Lightsail＋Docker Compose，PostgreSQL 与文件持久化卷 | ERP、供应商消息系统或复杂消息中间件 |

运费、税费、交期定义或规格不明确时，须先标记并确认；不能把未知费用默认为零，也不能把无法确认等价的商品直接比较。

补问只阻塞仍可能影响选择的报价或任务级必要条件。金额使用十进制定点运算；交期需要明确发货／到货、起算点和自然日／工作日。MVP 优先比较确认后的到货日期或明确起算日的自然日交期；无法处理的工作日规则要求用户确认到货日期。详细计算、来源核验与范围排除规则见架构文档。

## 5. 每周计划与 Milestones

### 5.1 三周总览

官网截至 2026-09-07 列出的开发阶段为 9 月 7–25 日，最终提交为 9 月 28 日；10 月 10 日为入围后的 Finale Demo Day，不是初次提交期限。具体提交时刻、渠道和格式以参赛通知为准。[官方比赛时间线](https://www.iss.nus.edu.sg/show-me-your-agents)

| 阶段 | 本周目标 | 主要交付物 | 团队总投入预算 |
| --- | --- | --- | --- |
| **第一周：9/7–9/13，能比较** | 带版本和来源跑通最小流程 | 样例、数据契约、版本骨架、提取与计算、纠正入口、最小部署 | 修订为 48 小时：38 小时计划工作＋10 小时缓冲 |
| **第二周：9/14–9/20，闭环完成** | 补问、更新、制度检索、供应商合规、审批和报告贯通 | 制度与供应商注册版本、合规矩阵、持久化恢复、审批与报告、AWS 回归 | 最新建议 80＋16 小时待确认，见 docs/WEEK2_PLAN.md |
| **最终周：9/21–9/25，稳定交付** | 固定功能范围，完成评测和提交包 | 保留集结果、故障修复、部署复核、演示与提交材料 | 40 小时：26 小时计划工作＋14 小时缓冲 |

**工作量口径：**暂按四人团队、1 人日＝8 小时估算。原总预算为 128 小时；采用本版 Week2 建议后三阶段合计为 184 小时（144 计划＋40 缓冲），仍待团队确认。最后一周仅五个开发日，成员需提前确认可投入时间；不能把团队总工时误解为个人工时。

9/26–9/28 只作提交缓冲，不安排新功能，也不假定额外人时；必要工作应使用上述剩余预算或调整范围。9/25 前准备可提交版本。若入围，再按通知准备 10/10 演示。

### 5.2 第一周：数据契约、版本骨架与最小部署

**本周目标：三份报价在本地后端形成可核对、可纠正、带版本的比较 JSON，并验证持久化中断恢复。** 具体以 [第一周计划](docs/WEEK1_PLAN.md)为准；React 和正式 RAG 运行链路第二周交付，云端验证按资源可用情况记录。

前两个工作日按 DATA.md 冻结 MCU 演示规格、源供应商映射、字段单位／精度、交易条款、工具契约和参考答案。采购需求、报价、人工修正和结果从首次持久化起携带任务 ID 与版本；同时建立 LangGraph 运行标识、PostgreSQL 检查点及数据库迁移骨架。

完成来源定位、提取候选值、字段核验、数量与金额计算及人工纠正，以 Swagger／脚本作为交互入口。CSV 使用明确模板；文本 PDF 聚焦已声明可支持的版式，复杂条款进入待确认。共同契约评审同时冻结 policy_set_version、条款／引用及检索状态格式；本周不要求检索实现。

先用团队自行配置的真实开发 API 完成本地闭环，具体服务商／协议待确定。Lightsail 与主办方 API 可用后尽早验证连通性、结构化输出和工具调用；正式访问未开放时继续本地工作，但云端验收保持未通过。若使用模拟接口则单独标识，AWS 失败不静默回退个人 API。

| 第一周交付 | 验收方式 |
| --- | --- |
| 样例与人工基线 | 源 CSV 与许可／哈希保存，MCU 合成报价有独立答案；按模板划分开发／留出集；测人工时间 |
| 统一契约与版本骨架 | 前端、解析、Agent 和计算工具使用同一字段定义；历史值不被覆盖 |
| 确定性比较与纠正 | 正确处理按盘／按颗、MOQ 和费用；纠正产生新版本并更新比较 |
| 来源与最小部署 | 原文定位可核验；本地 API 跑通三份报价；权限具备时在 Lightsail 用主办方 API 重复验证 |
| 持久化骨架 | PostgreSQL 业务表及检查点可恢复；Compose 重建后数据保留，API／worker 版本一致 |

**通过标准：**已知条件下计算正确，未知值不被猜测，字段来源可核对，刷新页面不会丢失已保存需求和纠正记录。

### 5.3 第二周：补问、更新、制度检索、供应商合规、审批与报告闭环

**本周目标：端到端完成“补问 → 初步价格与可行性比较 → 制度检索 → 批准供应商／RoHS 合规门禁 → 最终推荐 → 批准 → 报价更新 → 旧审批失效 → 重新核验与批准”。**

LangGraph 根据证据组织补问并持久化 interrupt，释放 worker。后端校验回答对应的问题、运行和版本，保存回答、推进修订号并创建恢复作业；恢复后读取最新确认数据，冻结新快照并完整重算。新报价／主动修正／范围修改创建新图运行，旧运行失效。实现检查点与业务提交崩溃窗口的幂等恢复；具体规则见架构文档。审批独立于图执行，首版不实现字段级依赖图。

| 第二周交付 | 验收方式 |
| --- | --- |
| 补问和恢复 | 刷新／重启后恢复持久化中断；重复回答／恢复幂等，过期回答不改当前版本，重试不重置调用预算 |
| 可行性与推荐 | 区分可行、不可行、待确认；未知不算零，不因已排除报价缺字段而阻塞其他报价 |
| 版本和并发控制 | 旧计算晚到不覆盖当前结果；旧页面审批被拒绝；重复请求不生成重复记录 |
| 审批和报告 | 只批准当前有效结果；重新检查有效期；报告绑定输入、范围、来源和审批 |
| 制度 RAG 与供应商合规 | 版本过滤、BM25／pgvector 混合召回、embedding／rerank API；批准供应商和 RoHS 精确查询；三态合规矩阵、引用及 React／HTML 展示通过开发与边界测试 |
| 完整演示闭环 | B 以 S$7,000 获批，交期更新后改荐 S$7,100 的 C，旧审批失效并解释 S$100 差额 |

**通过标准：**有阻塞问题时只显示草稿；无可行方案时不强行推荐；输入变化和报价自然过期都不能复用旧审批。

**当前建议为 80 小时计划工作＋16 小时缓冲。** 增量来自 embedding／rerank API、可信供应商身份、批准记录、RoHS 及合规矩阵；团队投入仍待确认。下表按第一周 A／B／C／D 延续责任，交叉评审与模块验证计入对应行。

| 成员 | RAG 工作／小时 | 其他闭环工作／小时 | 计划合计 | 缓冲 |
| --- | --- | --- | ---: | ---: |
| A | 制度、供应商注册与独立参考：6 | React 和报告展示：14 | 20 | 4 |
| B | embedding／rerank、制度检索、引用核验：12 | 提取回归与审核协作：6 | 18 | 4 |
| C | 合规规则和边界测试：6 | worker、认证、部署与 AWS：14 | 20 | 4 |
| D | 身份／注册版本、合规持久化与图集成：10 | 更新、审批、报告 API 与总集成：12 | 22 | 4 |
| **合计** | **34** | **46** | **80** | **16** |

以上估算以复用第一周稳定接口、成熟认证组件和现成 React 组件为前提，不是交付已完成的声明。第二周开始即冻结制度 control_code、供应商身份与注册表契约；中段完成“制度检索→结构化供应商查询→合规门禁→来源展示”纵向链路，后段与审批变更及 AWS 回归联合验收。

为容纳正式 RAG，首版采用结构化补问回答与偏好表单、固定问句；自由文本偏好、开放式多轮补问／润色、独立聊天界面、报告美化和额外开发 PDF 版式不列为必交付。最终评测保留独立版式。规则、审批、持久化恢复及 AWS 验证不删减；RAG 的版本过滤、可追溯引用和无依据处理同样必须保留。若首周基线不稳定或认证／恢复明显超出估算，记录实际差额并重排工作，不能把 RAG 改称可选项或暗中追加人时。

### 5.4 最终周：评测、稳定与提交

**本周目标：9/25 前交付一个可重复演示、结果可复核的提交候选版本。**

在 AWS 使用主办方 API 执行完整留出评测和回归，与本地开发 API 结果分开报告；修复关键异常，复核来源与报告，记录含人工的耗时、修正负担及调用成本。完善 Compose 版本／环境说明、备份恢复、演示录屏和提交材料；不在最后阶段首次接入云端模型。

| 最终周交付 | 验收方式 |
| --- | --- |
| 实际评测摘要 | 采购业务与 RAG 分开统计；完成 4 个 RAG 留出问题，注明样本量、检索召回、引用支持、人工修正前字段结果、时间和局限 |
| 回归与故障恢复 | 核心算术、补问、范围、版本、并发和时间边界用例通过 |
| 部署与运行说明 | 其他队员按说明启动；人工后备路径和模拟数据标识清楚 |
| 提交包与演示 | 已批准示例报告、可复现数据、录屏和材料与真实能力一致 |

**通过标准：**主流程和关键异常可重复执行；未通过的指标和未实现能力如实记录。最后阶段不扩大功能范围。

### 5.5 范围收缩顺序

进度不足时，依次缩减报告美化、自由文本偏好输入、字段级差异展示和额外 PDF 版式；需求可先使用结构化表单，报告可先导出 HTML。保留至少一类文本 PDF、CSV 模板、来源查看和人工纠正。

必须保留：确定性计算、缺失与冲突处理、补问持久化恢复、输入快照、旧结果和审批失效、制度检索、批准供应商／RoHS 合规门禁、简单报告及可运行演示。检索故障或合规证据不足时只能发布草稿；不得默认合规、忽略版本、用固定答案冒充检索或把人工结果包装成自动结果。

## 6. 数据、工具与运行约束

### 6.1 Data or Document

| 数据或文档 | 来源 | 所有人／责任方 | 当前访问状态 | 隐私或质量问题 |
| --- | --- | --- | --- | --- |
| 原始采购素材 | Procurement Spend Analysis Dashboard 合成 CSV，Electronics／MCU-9 子集 | 开源项目／团队 | 已在线核对；待下载归档，保留 MIT 许可与哈希 | 历史订单缺币种、完整规格和报价条款，不能当当前报价 |
| 供应商报价 | 团队基于 CSV 素材和固定种子生成文本 PDF／模板 CSV | 项目团队 | 待生成；真实报价可获授权后补充 | 新增字段明确标合成，隐藏参考答案，控制版式泄漏 |
| 采购需求 | 用户填写或团队设计的测试场景 | 采购用户／项目团队 | 每次比较时提供 | 规格、预算、数量和交付要求可能不完整 |
| 产品和供应商参考资料 | 产品目录及可获得的历史记录 | 数据提供方 | 可选，取决于可用性和授权 | 资料过期、记录不完整、无法支持表现判断 |
| 测试样例与参考答案 | 团队构建并人工核验 | 项目团队 | 第一周开始准备并持续补充 | 与调试样例过度重复，或标准答案本身出错 |

原始 CSV 共 47,128 条，Electronics 为 6,123 条，MCU-9 为 1,185 条／15 家合成供应商（2026-09-08 核对）。选 3–5 家素材建立约 20 个场景，缺失字段按种子与业务规则合成，参考答案独立核验并与运行输入隔离；必交付 1 种开发版式＋1 种独立留出版式。另制作 3–5 份虚构制度、A／B／C 的批准供应商与 RoHS 演示注册表、独立制度检索问题及合规矩阵。没有真实业务验证时，不称为已获 SME 客户验证。

### 6.2 AI Models & Tools

| 模型／工具 | 项目职责 | 运行约束 |
| --- | --- | --- |
| 本地开发 API→主办方 API | 理解报价、补问和解释；AWS 目标 Sonnet 4.6 或获准 Sonnet | 协议、模型 ID、认证及额度待核验，不预设 Bedrock；两环境分别测试，不静默回退 |
| LangGraph | 一个主图编排，interrupt＋PostgreSQL 检查点恢复 | 业务状态以业务表为准；审批经独立后端接口；恢复及重试幂等 |
| pdfplumber＋Python 业务模块 | 原文定位、字段理解接入、Decimal 成本和硬约束、报告 | Pydantic 校验结构，业务校验证据；不能猜测未知费用或替代型号 |
| FastAPI＋Pydantic＋React | 上传、纠正、补问、审批和版本界面 | 预建具名账户＋成熟认证组件／服务端会话；后端保存角色并验证审批身份 |
| PostgreSQL 16／pgvector＋SQLAlchemy／psycopg＋Alembic | 业务表、NUMERIC 金额、JSONB 快照、制度向量和迁移 | pgvector 精确余弦召回；检查点表独立管理；短事务和行锁；单 worker，作业表调度 |
| 官方 Lightsail＋Docker Compose | Web、API、worker、数据库及演示入口 | 镜像版本和环境配置固定；数据库／文件分别持久化，实例外备份及恢复验证 |

首版使用一个 LangGraph 主图及受控工具，不为展示多 Agent 增加角色，不训练模型或引入 RL。

采用模块化单体，API 与 worker 共用版本一致的后端镜像；前端按任务 ID 查询进度，模型工作通过持久化作业执行。业务表保存权威数据，检查点保存图执行位置，文件卷保存原件、解析文本和报告，均不向 Agent 暴露评测答案。

制度清单先按金额／品类／地区／时间确定必需 control_code，再融合 BM25 与 pgvector 精确余弦召回，并由 rerank API 重排支持条款；漏召回进入 REVIEW_REQUIRED，不能减少检查项。批准供应商状态和 RoHS 使用 PostgreSQL 版本化关系表精确查询；报价 supplier_id 必须映射到可信 supplier master。C 的确定性规则输出合规三态并按原价格／交期排序选择候选。提取、embedding、rerank 和解释分别设置持久化预算，恢复／重试不重置。

## 7. Integrations and Manual Fallback

首版只连接上传入口、Agent 工作流、比较工具和获批准的模型服务，不将实时 ERP 或供应商消息系统设为必要依赖。

| 异常 | 后备处理 |
| --- | --- |
| 文档解析失败或格式不支持 | 提示用户改用支持格式，或人工录入并核对关键字段 |
| 模型服务不可用或额度耗尽 | 有限重试后暂停；对已核验数据用确定性工具／模板并标记后备，AWS 测试不静默调用个人 API |
| 应用整体不可用 | 使用原始报价和标准比较模板，保留人工处理记录 |
| 关键信息缺失或冲突 | 按供应商判断影响范围；仍可能影响选择时补问并阻止最终批准，已有结果可作草稿查看 |
| 旧任务晚到、旧页面提交或重复点击 | 校验输入版本和幂等键；拒绝过期操作，不覆盖当前结果或重复创建审批 |

后备模式和人工纠正都应清晰标记，不把人工补全结果展示成模型首次自动完成的效果。

## 8. Success Measures：如何判断项目有效

以下是初步目标，尚未测量。第一周建立基线后可以调整；所有结果都应注明测试数据、样本量和测量口径。

| 指标 | 基线 | 暂定目标 | 测量方法 | 检查时间 |
| --- | --- | --- | --- | --- |
| 比较完成时间 | 第一周测量人工处理时间 | 在正确完成任务的前提下，较人工缩短约 30% | 对等任务计时，包含人工核对与修正 | 第二周及最终评测 |
| 关键字段提取准确率 | 第一周测量初版结果 | 约定测试集总体至少 90%，同时分项披露；总分不替代关键错误检查 | 使用人工修正前输出，分别统计金额、计价单位、包装、MOQ、费用和交期 | 每周 |
| 推荐有效性 | 第一周／第二周建立初版结果 | 测试中的最终推荐均满足已声明硬约束，并有关键依据 | 人工审查推荐、来源、缺失项及无可行方案情况 | 每周及最终评测 |
| 缺失／冲突识别 | 第一周标注参考问题 | 报告漏检数、误报数和召回率，第一周据基线确定目标 | 核对是否漏掉影响决策的问题，及是否发起无必要补问 | 第二周及最终评测 |
| 人工修正负担 | 第一周测量初版结果 | 报告无需修正任务比例、每任务修正次数和补问轮数 | 正常补问回答与提取错误修正分开记录；全部转人工不视为自动化成功 | 第二周及最终评测 |
| 来源正确性 | 人工标注原文依据 | 最终推荐的关键事实均有可定位且支持该事实的来源 | 分别检查定位有效性与语义支持，不能只检查是否存在页码 | 每周 |
| 运行效率 | 第一周记录初版耗时 | 披露系统处理时长和每任务模型调用成本，不预先宣称已达目标 | 记录调用次数、用量、失败重试；可计费数据缺失时明确标注估算 | 第二周及最终评测 |
| 异常和审批处理 | 首次实现后建立结果 | 预定的补问、更新、并发和审批测试全部通过 | 包括旧计算晚到、旧页面审批、重复请求、重启恢复和自然过期 | 第二周及最终评测 |
| 制度 RAG 与合规门禁 | 第二周建立 8 题 BM25／pgvector／rerank 检索基线和合规矩阵 | rerank Recall@3 宏平均至少 80%；引用支持目标 100%；供应商身份、批准状态、RoHS、三态与候选选择的预定边界全部通过 | 检索与结构化合规分开统计；实际分母、失败与两环境结果单列，详见 DATA.md 第 7 节 | 第二周及最终评测 |

逐步准备约 20 个带人工参考结果的业务场景，建议 12 个用于开发、8 个作为保留集。按报价版式或模板分组划分，不只对同一模板替换数字；保留集不用于调整提示词。若因排错查看并据此调优，应披露并补充新的保留样例。确定性计算和状态测试另行组织，不把每个代码断言计作一个独立业务场景。

场景覆盖 MCU 料号／封装缺失或不匹配、按盘／按颗和整包装取整、MOQ 超预算、未知／免费运费、交期歧义、并列、全不可行、仍待确认、用户排除、已不可行报价缺非必要字段、新版报价和自然过期。额外验证检查点／业务提交崩溃窗口、重复恢复、过期回答、并发审批、文档夹带指令，以及容器重建与备份恢复。

注意以下测量边界：

- “无可行供应商，正确请求人工处理”属于正确行为，不是必须避免的失败。
- 提取准确率应报告人工修正前结果，不能用修正后的数据冒充模型准确率。
- 原文已有字段按标准化值与单位判断正确；原文缺失项另报识别效果，避免大量空字段拉高提取准确率。语义字段的匹配规则在测试前固定。
- 计时使用难度对等的人工／系统任务，记录熟悉度与执行顺序，尽量交叉安排，避免重复同一报价带来的记忆优势。包括来源核对、补问和纠正，失败任务单列，不能从统计中静默删除。
- 测试集全部通过不代表对任意真实报价都保证正确；自建样例测试与真实业务验证需要分开报告。

## 9. 风险、控制与人工审批

### 9.1 Risks and Guardrails

| 风险 | 可能后果 | 预防控制 | 人工责任方 |
| --- | --- | --- | --- |
| 提取、换算错误或无依据解释 | 错误比较或推荐不合适的供应商 | 来源关联、确定性计算、输入核验及人工审阅 | 技术负责人／采购审核人 |
| 信息缺失或报价过期 | 基于不完整或失效条件决策 | 标记缺失项、检查有效期、记录版本并重新比较 | 采购审核人 |
| 商业信息泄露 | 价格、联系人或其他敏感信息暴露 | 优先使用匿名化样例、限制访问、减少不必要留存 | 数据责任人／技术负责人 |
| 文档中的指令或流程错误触发越权 | 未经批准的外部沟通或业务承诺 | 上传内容视为待分析的数据；限制工具权限；重要控制由后端执行 | 技术负责人／指定审批人 |

这些控制是拟实施措施，不代表已经完成验证。原型中的采购审核人与审批人可由指定队员模拟，不能表述为真实企业人员已经参与。

### 9.2 Human Approval Points

人工应确认重要字段纠正、假设及采购需求调整，并在最终定稿前审阅推荐、证据和未解决事项。

剩余比较范围内仍有影响选择的待确认报价，或任务级必要条件未解决时，不允许最终批准。已确定不可行或用户明确排除的报价可以保留未知字段，但不得把未知值写成已确认事实。用户可以查看或导出清晰标记的草稿，不能附有效审批标识。

报价或需求发生影响决策的变化后，旧结果和旧审批失效，需要重新比较和审阅。系统不自主联系供应商、下单或付款；批准的是当前版本的比选建议，不是对外执行采购的无限授权。

审批和最终导出需在后端重新检查当前输入快照及当时的报价有效期／交付条件；版本检查与审批写入在同一事务中执行。比较范围、人工修正、排序规则和计算规则版本也是决策输入。已下载报告无法远程撤回，须记载生成时间、适用时点、有效边界及输入版本；历史审批保留并附失效记录。

## 10. 最终要交付什么

**以下是团队建议准备的完整交付包，不等同于主办方已确认的逐项提交要求。正式文件格式、演示时长及提交渠道仍需核实。**

| 交付物 | 应包含的内容 | 最低完成标准 |
| --- | --- | --- |
| **可运行的 Web 原型** | 上传、比较、补问、推荐、更新、审阅和审批 | 至少一个正常案例和关键异常案例可以端到端完成 |
| **源代码与运行说明** | 前后端代码、依赖、配置示例、运行与部署步骤 | 其他队员能按说明启动；配置示例不包含真实密钥 |
| **样例数据与测试材料** | 允许分享的报价、采购场景、人工参考答案与测试说明 | 数据来源和虚构／匿名化状态清楚，结果可复查 |
| **示例决策报告** | 采购需求、比较范围、成本明细、推荐及排除理由、来源、版本、时间边界和审批记录 | 首选可导出 HTML；未批准草稿与已批准报告明确区分 |
| **制度 RAG 与供应商合规** | 3–5 份虚构制度、BM25／pgvector＋embedding／rerank API、批准供应商／RoHS 注册数据、三态合规矩阵、引用展示和独立评测 | 主演示命中适用条款并精确查到注册事实；无依据、冲突、过期和版本边界达到 DATA.md 第 7 节标准 |
| **评测结果摘要** | 人工基线、系统结果、异常测试、耗时及已知限制 | 使用实际测量值，不把目标写成已实现效果 |
| **展示与提交材料** | 演示脚本、项目介绍、流程和业务价值说明；按要求准备录屏或演示稿 | 与真实原型一致；模拟事件和未实现功能明确标注 |

最终成品不要求做成生产级采购系统。它需要证明：**报价能被正确整理，建议有依据，条件变化能被处理，重要决定由人确认。**

## 11. 建议演示案例

共同演示为 MCU-DEMO-001，完整参考以 [DATA.md 第 5 节](docs/DATA.md#5-共同主演示与参考答案)为准。全部是团队合成条款，不能表述为原始 CSV 中的当前报价。制造商 QQ Demo Components、料号 QW-MCU9-DEMO、QFN-32／R1 为虚构演示规格，全新且不允许替代；SGD，明确含运费、不计税且无其他费用。

采购要求：**1,000 颗 MCU，总预算 S$8,000，以总成本最低优先。** 受控时钟为 2026-09-14 09:00（Asia/Singapore），当日下单为第 0 日，2026-09-19 前到货。初始报价日期为 9/13，有效至 9/20 当日结束，付款条件为演示 Net 30。测试日期固定并明确标识；实际应用使用真实时间，不自动移动旧报价日期。

| 供应商 | 报价 | MOQ／订购倍数 | 运费 | 到货周期 | 参考结果 |
| --- | --- | --- | --- | --- | --- |
| A | S$640／盘，每盘 100 颗 | 20 盘／整盘购买 | 免费 | 7 天 | 2,000 颗，S$12,800；超预算且超期 |
| B | S$6.80／颗 | 1,000 颗／可按颗购买 | S$200 | 3 天 | 1,000 颗，S$7,000；可行且最低 |
| C | S$660／盘，每盘 100 颗 | 10 盘／整盘购买 | S$500 | 3 天 | 1,000 颗，S$7,100；可行 |

上表是完整条件和人工参考答案。正式演示以 B 的运费缺失版本开始，按以下连续流程展开：

1. 上传三份报价。A 因 MOQ 成本和交期被排除；B 待确认；C 可行。系统展示草稿，暂不宣称 C 最优。
2. Agent 请求确认 B 运费，引用相关上下文而不伪造缺失条款。用户回答 S$200 并确认，后端保存人工输入、推进版本并排队恢复对应 LangGraph 中断。
3. 新快照重算 B 为 S$7,000、C 为 S$7,100，B 成为初步价格第一名，但尚未发布最终推荐。
4. 系统检索适用的金额门槛、批准供应商和 RoHS 条款，并从当前 supplier registry 精确查询 B 的批准状态与有效 RoHS；确定性门禁判定 B 为 COMPLIANT 后发布推荐，审核人批准并导出报告。
5. 同一受控场景日上传 B v2，报价日期为 9/14、有效期不变；到货改为 6 天、明确运费仍为 S$200。新版独立核验，不继承旧人工回答。
6. 旧图运行、结果、合规 assessment 和审批失效；新运行按交期改荐 C，重新检索制度并查询 C 的注册事实，解释增加 S$100 满足五天到货要求，并请求重新批准。
7. 展示制度文档版本／章节／原文，以及供应商注册表和 RoHS 记录版本；S$100 来自比较工具，不从制度或资质文本推断。

另备短场景：A 已确定交期超限但缺运费，不追问无助于决策的信息；A、B、C 都已确定超限时输出“无可行方案”；若 A、C 超限而 B 的交期未知、也没有其他已确定的失败项，则输出“暂时无法确定”。演示中的模拟更新和人工回答清楚标识。

## 12. 团队分工建议

沿用第一周的 A／B／C／D 标识，第二周正式工时见第 5.3 节；实名映射由团队内部确定。

| 角色 | 主要职责 |
| --- | --- |
| A：数据、制度与交互 | MCU 样例；制度、供应商注册和 RoHS 演示数据；React、报告模板和验收参考 |
| B：解析、模型与检索 | pdfplumber、模型适配、证据核验；BM25／pgvector、embedding／rerank 制度检索、引用核验及测试 |
| C：规则、作业执行与部署 | Decimal 与合规三态规则；worker、认证、Lightsail／Compose 与 AWS 回归 |
| D：业务版本与图流程 | API、迁移、供应商身份／注册版本、LangGraph、快照、审批、检索轨迹和报告 |

开始并行开发前，按已定 MCU-9 场景共同冻结具体规格、字段结构、工具输入输出和演示参考答案。规则和验收由团队共同确认，避免各模块采用不同口径。

数据库和 API 契约由 D 维护，其他模块共同评审。第二周 RAG、版本、审批均为正式工作，检索模块 B 与图集成 D 从周初联调，A／C 负责内容及规则交叉核验；每周按实际工时调整分工，不把集成责任全部留给某一人。

## 13. 开始开发前需要确认的事项

| 待确认项 | 需要明确什么 |
| --- | --- |
| 数据细节 | 按 DATA.md 冻结演示规格、源供应商映射、字段精度、模板与独立答案；具体值调整需同步三份文档 |
| 模型接入 | 本地 API 服务商／协议；主办方 API 模型 ID、认证、地址／区域（如适用）和额度 |
| 比赛适用性 | LangGraph 是否符合参赛要求；Lightsail 访问方式、CPU／内存／磁盘／系统架构及有效期 |
| 实施参数 | 认证组件、部署域名／HTTPS、迁移版本、连接池、超时及保留／备份期限；选型本身已确定 |
| 数据权限 | 保留合成数据许可；真实报价仅在获授权后补充，不阻塞首版 |
| 团队投入 | 实际成员分工、每周可投入时间及工时估算是否需要调整 |
| 官方提交要求 | 官网日期已核对；仍需确认 9/28 的具体提交时刻、渠道、演示形式及文件格式 |

FastAPI／Pydantic／React、LangGraph、PostgreSQL／pgvector、pdfplumber、制度 RAG＋结构化合规门禁、Lightsail／Docker Compose 与持久化卷已确定。制度检索采用 BM25＋pgvector＋embedding／rerank API；批准供应商和 RoHS 采用精确查询；ISO 和框架合同不进入 Week2。模型路线为本地开发 API→主办方 API，具体接入信息仍待核验。

## 附录 A：可用于 Proposal 的英文 Solution Overview

> We propose a supplier comparison assistant for SME electronics procurement, initially comparing quotations for one fixed-specification MCU with no substitute parts. FastAPI, Pydantic and React support the application, while a LangGraph workflow interprets documents, asks targeted questions and resumes through PostgreSQL checkpoints. Independent Python tools calculate quantities, costs and feasibility; backend transactions enforce versions and human approvals. PostgreSQL business records remain authoritative. Synthetic quotation PDFs are generated from open synthetic procurement data and parsed with pdfplumber, with evaluation answers isolated from agent inputs. Development uses a team-configured model API, followed by integration and regression testing against the organizer-provided API on the supplied Lightsail instance. Docker Compose manages services and environments, with persistent database and file volumes. Model access details remain subject to confirmation. Evaluation separates local and AWS results and measures field and evidence accuracy, clarification burden, cost and completion time. The system does not contact suppliers, place orders or make payments.

英文 Proposal 的正式 RAG 交付说明：

> Procurement-policy RAG and a deterministic supplier-compliance gate are required MVP components. Version-filtered BM25 and pgvector retrieval are fused, then an API reranker selects applicable fictional policy clauses. Approved-supplier and RoHS facts are queried exactly from a versioned PostgreSQL registry, rather than inferred from similarity scores. Deterministic controls produce COMPLIANT, REVIEW_REQUIRED or NON_COMPLIANT outcomes before a recommendation can be published. The interface and report show both policy citations and registry evidence; retrieval and structured compliance are evaluated separately.

## 附录 B：资料依据与状态

本文件依据本次对话中用户提供的选题截图（Supplier Comparison 条目）、Scope and Proposed Approach 表单、Delivery, Measurement and Controls 表单，以及围绕这些内容形成的团队方案建议整理。

2026-09-07 修订时补充核对 [NUS-ISS 官方比赛页面](https://www.iss.nus.edu.sg/show-me-your-agents) 的开发与提交日期。公开页面未列出具体 Agent 框架限制；本文不据此推断任意框架均获许可。运行架构和状态契约见 [ARCHITECTURE.md](docs/ARCHITECTURE.md)。

2026-09-08 按团队已收束方案同步商品和工程选型，并在线核对 Electronics／MCU-9 数据规模。产品名称、人力投入、性能目标及具体配置仍可调整；本文不认定真实企业数据、企业审核人、模型权限或代码实现已到位，也不将建议交付包视为官方强制清单。

---

## 项目主线

**第一周：把不同报价变成一张带来源和版本的可信比较表，并贯通最小部署。**

**第二周：补问后继续比较，处理报价变化，完成审批与报告闭环。**
**制度 RAG 与供应商合规同期交付：推荐展示可追溯制度引用、批准供应商状态和 RoHS 证据，并分别评测检索与结构化合规。**

**最终周：用保留样例验证效果，修复问题，完成稳定演示和提交。**
