...
2. 整理 `quote_V1` 至 `quote_V7` 测试数据，覆盖基准输入、现实字段差异、31 字段直接表达、极端边界、分集泛化和 OCR 路由测试。
3. 建立版本化参考答案；V2 至 V6 已提交完整答案。V7 只提交开发、校准答案和留出答案哈希，运行时不得读取任何参考答案。
...
- V2、V3、V4、V5、V6 答案已提交；V7 开发/校准答案已提交，留出答案只提交哈希承诺；V1 答案仅本地保存。
...
- 三份 1.2.0 数据字典及来源说明。
- V1–V7 测试输入的整理和版本目录；V6、V7 已隔离开发、校准和留出集。
...
- Week 1 原基线只支持英文可提取文本 PDF 和批准 CSV。V7 是后续 OCR 升级的评测数据与契约准备，不代表 OCR 解析器已经实现或通过。
- 默认单 PDF 最多 5 页/5 MB，单任务最多 5 份；限制必须可配置。
- 截至本次提交，OCR 仍是 B 待实现并验收的能力；V7 仅提供测试数据。任意 Excel、阶梯价格、复杂折扣/税务、自动汇率、分批交付和替代料兼容判断仍不在范围内。
...
│   │   └── holdout/quote_V6/、quote_V7/
│   ├── fixtures/quote_V4/
│   └── manifests/quote_V7_manifest.json
evaluation/
└── reference/
    ├── quote_V2/
    ├── quote_V3/
