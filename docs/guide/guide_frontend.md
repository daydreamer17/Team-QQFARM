...
范围：任务中心、采购需求、报价草稿审核、决策比较、Policy 资源与合规证据、版本审计及 Summary 占位页
...
- 新增 Summary 页面，但只作为真实后端报告能力的明确占位，不生成假报告。
...
采购需求文件的“解析并自动填入”目前仍是前端原型模拟，附件不会上传或持久化；页面已经对此作出明确标注。概览页的“修改采购需求”和“废弃任务”按钮也保持禁用，等待后端接口。
...
历史正式报价的元数据可查看；由于后端尚未提供带权限控制的文件内容流，历史原件“预览”只显示说明，不会伪造文件内容。
...
规则资源库已经从本地模拟数据切换为真实后端流程。

策略上传支持：

- 单个 PDF 或 UTF-8 TXT，最大 5 MiB。
- Policy Set、Policy、Document 的 ID 和版本。
- 标题、生效时间、分类和地区。
- 上传成功后进入真实策略审核详情页。
...
已发布 Policy 目录展示可绑定的 Policy Set、索引版本、文档数、条款数、Embedding 模型和发布时间。新建任务只能从这里选择已发布版本，不能手填或自动选择“最新版本”。
...
`Summary` 已替代原“审批 / 报告”命名。当前页面会说明未来报告范围，并在有结果时链接到当前决策结果；“生成 AI Summary”按钮保持禁用，明确标注后端报告接口待接入，不生成假报告，也不暗示已经完成审批。
...
| `frontend/src/pages/SummaryPage.tsx` | AI Summary 明确占位页 |
...
以下功能在界面中只提供占位或被明确禁用，下游不能把它们视为已完成：

| 功能 | 当前状态 | 建议后端能力 |
| --- | --- | --- |
| 采购需求文件解析 | 前端原型模拟，附件不上传 | 需求文档上传、解析产物、证据和版本化接口 |
| 修改采购需求 | 按钮禁用 | 带 `expected_task_revision` 的修改接口和快照重算 |
| 废弃任务 | 按钮禁用 | 可审计的任务废弃接口和权限检查 |
| 历史报价原件预览/下载 | 只展示元数据和说明 | 带权限、审计和正确 Content-Type 的文件流接口 |
| AI Summary | 页面及范围说明已完成，按钮禁用 | 版本化报告作业、状态、存储、读取和失败重试接口 |
| 任务中心全量检索 | 当前仅浏览器内处理最近任务 | 服务端搜索、筛选、排序和分页参数 |
...
1. 实现版本化 AI Summary 后端契约和持久化，再启用 Summary 按钮。
2. 提供历史报价文件的鉴权下载/预览接口，补齐证据查看闭环。
3. 实现采购需求文件真实解析，并让需求证据与任务快照一起版本化。
4. 实现采购需求修改和任务废弃，确保输入变化触发新快照、旧结果失效和草稿过期。
5. 数据量增大后，将任务搜索、筛选、排序和分页下沉到后端。
