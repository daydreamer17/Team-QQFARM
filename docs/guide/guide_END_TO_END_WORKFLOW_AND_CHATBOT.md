> **当前范围（2026-09-24）**：当前全流程包含结构化准入与 RoHS 数据录入和制度检查；真实性验证、外部系统查询和金额审批记录不包含在内。
...
本 Guide 描述的是当前 MCU-9 演示范围，不代表已经支持任意物料和采购模式。

| 范围 | 当前边界 |
| --- | --- |
| 商品 | 单任务比较一种固定规格的 `Electronics / Microcontroller MCU-9`；演示料号为 `QW-MCU9-DEMO`，不判断替代料的电气、引脚或固件兼容性 |
| 供应商 | 主流程按每个任务 3–5 家供应商设计；新供应商可以上传报价，但不保证存在历史表现 |
| 历史表现 | 当前发布是版本化的 MCU-9 合成历史。发布集包含 15 家供应商；`full_flow_demo3` 的 5 家报价供应商中只有 4 家历史状态可用，`SUP-030` 为样本不足 |
| 历史适用性 | 只有商品范围、供应商身份和数据版本均匹配时，才能使用评级、历史准时率和历史拒收订单行率；缺失历史不能记为 0，也不能使报价自动不合格 |
| 文件 | 主流程覆盖英文文本型 PDF、固定模板 CSV 和人工录入；扫描件、任意 Excel 或未知版式不属于稳定保证范围 |
| 计算 | 主要覆盖 SGD、固定单价、明确计价单位、简单 MOQ/包装、一次交付和明确费用；不自动处理阶梯价、复杂折扣、分批交付或实时汇率 |
| 日期 | 支持明确到货日或信息完整的简单交期；只有“若干工作日”但没有可用日历或换算依据时保持待确认，不能偷换成自然日 |
| 制度 | 只使用任务明确绑定的制度、供应商注册表及其版本，不查询实时法规，也不把制度检索结果当成人工采购批准 |
| 数据性质 | MCU-9、器件规格、供应商、报价和历史记录均为合成演示数据，不是实时市场、信用或真实采购事实 |

历史采购记录只用于计算历史表现，不能替代本次报价中的价格、付款条件、运费或交期。任务超出 MCU-9 范围时，历史指标应显示“不适用”，而不是套用相似名称的数据。
...
```mermaid
flowchart TD
    B[上传、审核并发布制度<br/>可选] --> A[创建并确认采购需求<br/>创建时选择已发布制度版本]
    A --> C[上传报价文件]
...
制度必须先发布，才能在创建任务时绑定；现有任务不会因新上传制度而自动切换依据。制度检索命中不等于供应商通过合规、RoHS 或金额审批，缺少供应商事实时仍需人工核验，不能把它描述为已执行完整的制度硬约束判定。
