...
→ 后端确定性预检
→ 用户对照报价原件，确认或修改全部 30 个字段
→ 前端执行字段和关联校验
→ 一次性提交 30 个字段的人工动作
→ 后端在同一事务中应用动作并权威复核
→ 复核通过后进入 READY_TO_SUBMIT
→ 用户点击“正式提交报价”
→ 后端重新加载最新 Batch、人工事件、字典和采购需求再次复核
...
| `SUBMITTED` | 已写入正式报价历史 | 只读查看、预览和下载 |
...
4. 确认是否为合成测试数据。
5. 点击“上传并开始审核”。
6. 保持 API 和 Worker 运行，等待草稿从 `PROCESSING` 进入 `REVIEW_REQUIRED`。

### 9.2 人工确认 30 个字段

1. 点击“预览 / 下载原件”。
2. 按身份、规格、价格、包装、MOQ、费用、交期和商务分组检查。
3. 查看每个字段的：
   - LLM/解析候选值。
   - 原文值。
   - 验证状态。
   ...
   - 当前必填级别和预检错误。
4. 值正确时保持不变；值错误时直接修改。
5. 可选或不适用字段未提供时，保持为空，系统会生成 `CONFIRM_MISSING`。
6. 检查完成后，点击“确认并复核 30 个字段”。

此操作只执行人工确认和后端复核，不会立即写入正式报价。
