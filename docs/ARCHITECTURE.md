...
> v0.2 · 2026-09-07 · 设计审查修订稿。交付范围、时间线和业务评测见 [项目方案](../Supplier_Comparison_Project_Plan.md)。
...
```mermaid
flowchart TD
    UI[Web 界面] --> APP[应用服务与持久化状态机]
    APP --> JOB[后台作业执行器]
    JOB --> AG[一个主 Agent]
    JOB --> PARSE[解析与字段核验]
    AG --> READ[证据读取与补问工具]
    AG --> COMP[确定性比较工具]
    AG --> MODEL[Bedrock 模型适配器]
    APP --> REVIEW[审批与报告模块]
    APP --> DB[(业务数据库)]
    PARSE --> FILES[(原始文件与解析文本)]
    PARSE --> DB
    READ --> FILES
    READ --> DB
    COMP --> DB
...
首版采用模块化单体：一个后端代码库维护工作流、Agent、解析、计算、审批和报告模块，可部署在同一应用实例；Web 通过 API 访问。模块划分不要求拆成微服务。业务数据库保存权威任务状态和审计记录，文件存储保存原件与解析文本；Agent 会话只保存上下文和中断信息，不能替代业务状态。

模型任务通过持久化作业记录后台执行，前端按任务 ID 轮询进度即可。MVP 可使用数据库作业表和单执行器，无需额外消息中间件。同一任务的写操作需串行化或使用版本条件更新；进程重启后可以识别未完成作业并按幂等规则恢复。

工具优先实现为同进程函数，不引入独立 MCP 服务、向量数据库或多 Agent 服务作为必要依赖。具体框架、数据库、解析器和 AWS 服务在第一周前两个工作日按权限及团队经验确认，并记录选型依据；本节不表示它们已经部署。
...
若选择 Strands，可评估其原生中断与会话持久化能力；恢复 Agent 上下文后仍须从业务数据库读取当前版本。[中断文档](https://strandsagents.com/docs/user-guide/concepts/interrupts/)、[会话管理文档](https://strandsagents.com/docs/user-guide/concepts/agents/session-management/)
...
| `Job` | `job_id`、`task_id`、`task_revision`、`snapshot_id`（有则记录）、阶段、`QUEUED/RUNNING/SUCCEEDED/FAILED`、幂等键、尝试次数、心跳或租约、错误类型 |

金额在 API／持久化中使用十进制字符串或精确 decimal 类型，数量和包装单位显式表示。必填与可空规则由 schema 定义；费用 `UNKNOWN` 与金额 `0` 不等价。关键事件日志记录工具、输入／结果 ID、状态、耗时和错误，不把完整敏感报价无差别复制到日志。
...
任务修订号、输入快照和 Agent 会话 ID 用途不同，不得互相代替。补问后的自然语言回答若解析成候选值，必须经结构化确认后才提交正式修改。
...
作业以任务、输入版本和阶段识别。领取作业、状态写入和结果发布要有条件更新；同一任务不得有两个执行器无约束写当前状态。使用数据库锁或乐观并发控制即可，具体方式随数据库确定。

瞬时服务错误采用有限次数和退避重试，语义缺失／冲突直接补问，不靠无限重试猜答案。首版可暂设单阶段最多 3 次尝试、单次 Agent 运行最多 8 次模型调用，并配置超时和预算；这些是可调整的保护阈值，不是性能承诺。超限保存当前状态并转人工处理。
...
以下字段必须分开保存：基本计数单位（如个）、每包装包含的基本单位数、允许的采购步长、MOQ 数量及单位、价格金额、价格所覆盖的单位和数量。按箱与按百个计价不能仅靠一个 `unit_price` 数值区分。
...
按整箱采购时 `S` 为每箱个数；允许按个采购时 `S = 1`。例如需求 1,050 个、MOQ 1,000 个、每箱 100 个且须整箱购买，则采购 1,100 个（11 箱）。如果 MOQ 原文为 20 箱，先按每箱数量换算，不能把 20 当作 20 个。

设固定价格 `P` 覆盖 `B` 个基本单位，则货款为 `Q / B × P`。必须有证据说明该计价规则适用于实际采购量：按箱销售须满足整箱限制；“每百个”的价格能否用于非整百采购无法确认时先补问。用规则化字段转换口径，保留原始表达和换算明细。
...
以下与项目方案的虚构演示采用相同商品规格、SGD、不计税简化口径、已确认的共同起算日和自然日到货定义。若用户需要在五天内采购 1,000 个包装盒，预算不超过 S$300，并在满足条件后选择总成本最低的方案：
...
首版至少区分任务访问者与指定审核人身份，在服务端校验文件、任务和审批的归属；可由队员模拟审核角色，但不能仅相信客户端传入的用户名。密钥由部署环境提供，模型和文件存储权限限于项目所需操作。具体认证实现、加密、网络隔离、保留期限和 AWS 权限配置在第一周部署选型时记录，不把待实施控制描述为已经上线。
...
| 恢复与故障 | 补问后刷新／重启可恢复；过期问题回答被拒绝；作业重试有上限；后备模式有标记 |
...
| 语言、Agent 框架、Bedrock 模型、调用额度和超时／调用上限 | Agent 与业务流程负责人 | 第一周前两个工作日；受比赛权限约束 |
| PDF 解析器、CSV 模板、核验规则和文件大小／页数上限 | 文档与计算工具负责人 | 第一周前两个工作日冻结首版范围 |
| 数据库、文件存储、作业恢复和并发控制实现 | Agent 与业务流程负责人、测试与部署负责人 | 第一周前两个工作日选型，第一周跑通骨架 |
| AWS 服务、身份认证、访问控制和数据保留配置 | 测试与部署负责人 | 第一周最小部署时记录并验证 |
| 真实报价使用与展示权限 | 数据责任人／团队 | 引入真实数据前；未获许可继续用标记明确的样例 |

已明确的首版决策不再列为开放项：一个主 Agent、模块化单体、业务数据库为权威状态、不可变快照、决策输入变更全量重算、纯视图设置免重审、HTML 优先导出。具体软件选型未决定不妨碍按契约实现业务工具，但必须在真实集成前完成确认。
