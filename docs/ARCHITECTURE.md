...
2. 制度导入时调用 embedding API 生成条款向量，并以固定维度的 `vector(dim)` 保存到 pgvector；查询使用相同模型／维度及正确的 query 输入模式。步骤 1 的结构化条件先在 SQL 中过滤，再并行执行 BM25 稀疏召回和 pgvector 精确余弦 Top-10，融合候选后调用 rerank API 取 Top-3 供解释；候选不足则取实际数量。首版数据量小，默认使用精确检索，不预建 HNSW／IVFFlat；只有基准测试证明延迟超标时才引入近似索引。缓存键包含集合／条款哈希、provider、embedding 模型版本、维度与预处理版本；变更时写入新索引版本并完整重建，全部完成才可启用。重排输出只可映射到本次候选 ID。
