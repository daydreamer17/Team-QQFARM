> **当前范围（2026-09-24）**：最小版本支持结构化供应商准入记录和 RoHS 证书输入，并结合 RAG 制度引用输出检查结论；真实性验证、外部 APL/注册表集成和金额审批记录仍不实现。

# 供应商比选系统架构

> v0.5 · 2026-09-14 · 制度 RAG＋结构化供应商合规门禁。交付与时间线见 [项目计划](../Supplier_Comparison_Project_Plan.md)，数据与共同演示见 [DATA.md](DATA.md)，选型记录见 [方案.md](方案.md)。

## 架构状态与适用范围

本文描述 Supplier Comparison 项目的**目标架构**，用于指导原型开发、测试和演示，不表示功能已经实现。已确定 FastAPI＋Pydantic＋React、LangGraph、PostgreSQL／pgvector、pdfplumber，以及主办方 Lightsail 实例＋Docker Compose／本地持久化卷。模型先使用团队本地开发 API 跑通，再切换主办方 API 做 AWS 部署测试；具体协议、模型 ID、认证和实例资源参数待核验。
...
- 制度 RAG 与供应商合规门禁为正式模块：语义检索适用制度，精确查询批准供应商和 RoHS，确定性规则控制最终推荐资格。
...
系统接收采购需求、供应商报价、用户对缺失或冲突信息的补充，以及团队维护的版本化采购制度和供应商注册表。报价形成采购事实；制度定义适用控制，注册表提供批准状态和 RoHS 事实。系统产生结构化报价、待确认问题、确定性计算结果、供应商可行性／合规判断、带引用的推荐建议和最终审批记录。

系统中的判断权分为三层：

1. **模型层**负责理解文档、识别可能的字段与歧义、生成补问和解释结果。
2. **确定性业务层**负责单位换算、采购数量、总成本、硬约束、供应商合规三态、版本和审批有效性。
...
flowchart TD
    ...
    AG --> READ[证据读取与补问节点]
    AG --> COMP[确定性比较模块]
    AG --> RET[制度版本过滤、BM25＋pgvector 召回与 rerank 重排]
    RET --> DB
    AG --> REG[供应商身份与批准／RoHS 精确查询]
    REG --> DB
    RET --> GATE[确定性合规门禁]
    REG --> GATE
    COMP --> GATE
    GATE --> EXPL[带引用解释与引用核验]
    EXPL --> MODEL
    AG --> MODEL[统一模型适配器]
...
工具优先实现为同进程 Python 函数。当前报价直接解析，历史查询使用 SQL，采购制度通过正式 RAG 模块检索；条款稠密向量存入现有 PostgreSQL 的 pgvector 扩展，不部署独立向量数据库、MCP 服务或额外消息中间件。SQLAlchemy＋psycopg 访问 PostgreSQL，Alembic 管理业务表迁移和 pgvector 扩展启用；检查点初始化与版本单独管理。Pydantic 校验结构，业务代码校验证据、规格和交易条件。
...
| `query_supplier_registry(snapshot_id, supplier_master_ids)` | 查询冻结注册表中的批准状态和 RoHS | 只接受已确认的规范供应商身份；按版本和评估时点精确过滤，保留来源 |
| `evaluate_compliance(snapshot_id, retrieval_id, registry_evidence_ids)` | 生成所有可行候选的合规矩阵 | 只执行审阅后的 control_code；不接受模型提供的新规则或事实 |
...
LangGraph 主图依次处理输入解析、字段理解、校验／补问、确定性比较、retrieve_policy、供应商注册记录查询、合规门禁和 explain_with_sources；阻塞问题可以 interrupt，但不能先宣布最终推荐。补问回答后回到业务校验并重算。推荐通过发布检查后结束图运行，人工审批通过独立 FastAPI 接口处理。检查点使用 PostgresSaver／AsyncPostgresSaver 等持久化实现；不依赖仅存内存的检查点完成重启恢复。[LangGraph 持久化](https://docs.langchain.com/oss/python/langgraph/persistence)

### 采购制度 RAG 与供应商合规门禁

这是第二周必交付模块。首版知识库为 3–5 份团队编写的英文 Markdown 制度，逐章节切分并保留稳定条款 ID；正文和清单从版本化文件导入 PostgreSQL，导入幂等且不覆盖旧版本。制度均标记为虚构演示资料，字段和评测隔离见 DATA.md 第 7 节。首版供应商合规只覆盖批准供应商状态和 RoHS；ISO 与框架合同不进入 Week2。

1. 后端根据固定的 policy_set_version 及制度清单中的金额／品类／地区／时间范围，先确定本任务必需的 control_code，再筛选允许检索的条款；不让 Top-k 结果决定需要执行哪些规则。原始问题与控制码形成英文检索输入，中文按钮可映射固定英文问题。
2. 制度导入时调用 SiliconFlow `BAAI/bge-m3` 生成条款向量，并以固定维度的 `vector(1024)` 保存到 pgvector；查询使用同一模型和维度。步骤 1 的结构化条件先在 SQL 中过滤，再并行执行 BM25 稀疏召回和 pgvector 精确余弦 Top-10，以 RRF `k=60` 融合候选后调用 `BAAI/bge-reranker-v2-m3` 取 Top-3 供解释；候选不足则取实际数量。首版数据量小，默认使用精确检索，不预建 HNSW／IVFFlat；只有基准测试证明延迟超标时才引入近似索引。索引身份包含集合／条款哈希、provider、embedding 模型版本、维度与预处理版本；变更时写入新索引版本并完整重建，全部完成才可启用。重排输出只可映射到本次候选 ID。
3. 每条可执行制度条款绑定团队审阅的 `control_code` 和参数。后端检查必需控制码均有当前版本且语义支持的条款命中；漏召回时进入 REVIEW_REQUIRED，不能少执行检查后判定合规。模型只收到当前已确认事实、计算结果及实际检索条款；它不能创建控制码、阈值或规则。解释的每条主张绑定文档／版本／条款 ID、原文和哈希。
4. 检索记录状态 OK／NO_EVIDENCE／CONFLICT／ERROR；无合格候选或不足以支持回答时不硬拼答案；召回／重排阈值只在开发集校准，分数不是概率或语义支持证明。发现版本或内容冲突时明确提示，不由模型选择较宽松规则。检索文本中的指令不能取得工具或审批权限。NO_EVIDENCE 与 ERROR 分别表示无依据和运行故障。
5. 检索故障不改变金额或价格排序，但合规状态为 `REVIEW_REQUIRED`，只允许展示草稿，不发布最终推荐或进入最终审批。正式项目验收必须包含成功检索、正确引用和失败处理，不能以运行降级替代交付。
...
供应商身份与制度检索分开处理。报价上传时的 `supplier_id` 只是客户端声明，必须映射到版本化 `supplier_master_id`；未匹配、多个候选或名称冲突时创建结构化问题。批准供应商状态、RoHS 证书编号和有效期保存在 PostgreSQL 关系表，按 `supplier_registry_version` 和评估时点精确查询，不放入向量召回来决定事实真伪。每条结构化记录仍保留来源文件／行、版本和内容哈希，供 React 与报告追溯。

确定性合规引擎一次评估所有 `FEASIBLE` 候选，输出 `COMPLIANT`、`REVIEW_REQUIRED` 或 `NON_COMPLIANT` 及原因码。高排名候选仍待审核时阻止最终推荐；明确不合规后才可选择下一名合规候选；全部明确不合规时返回 `NO_COMPLIANT_SUPPLIER`。人工补证创建新注册记录版本，身份或证据确认保存 `ComplianceReviewEvent`；它不等于最终审批，首版不提供无证据的任意合规豁免。

模型预算分开管理：每文档提取最多 8 次；制度导入 embedding 按批次记账；图运行持久化 query embedding、rerank 和 LLM 解释／支持判断的尝试次数、耗时及用量，各自上限在 Week2 接口冻结时确定。重试计入预算，恢复不重置。LLM、embedding、rerank 分别配置 endpoint／模型／Key 环境变量／超时与重试，不能假定主办方聊天 API 同时提供另两类能力。任一检索 API 故障记录 ERROR，不静默跳过重排或换检索器冒充成功；本地和 AWS 以完整模型组合分别验收。首版由工作流产生固定查询，不新增独立聊天系统或无限问答循环。
...
| 供应商主数据与注册表 | 规范化供应商身份、批准状态、RoHS 记录、来源、有效区间和不可变注册表版本 |
| 合规评估 | 每个可行候选的制度控制码、批准／RoHS 检查、三态结论、原因码及证据版本 |
...
| `InputSnapshot` | `snapshot_id`、`task_revision`、需求／报价／字段版本、比较范围及用户排除、偏好、`schema_version`、`calculation_rule_version`、`policy_set_version`、`supplier_registry_version`、时间基准、内容哈希；创建后不可修改 |
| `ComparisonResult` | `result_id`、`snapshot_id`、逐供应商数量与费用明细、约束 `PASS/FAIL/UNKNOWN`、可行性、事实与来源 ID、排序及并列、阻塞项、计算时间与时间有效边界 |
| `SupplierMaster / SupplierRegistry` | 规范化供应商 ID、别名、注册表版本、批准状态、RoHS 编号／范围／有效区间、来源记录及内容哈希；客户端声明不能直接成为可信身份 |
| `ComplianceAssessment` | `assessment_id`、任务／修订／供应商、`policy_set_version`、`supplier_registry_version`、`retrieval_id`、控制码检查、`COMPLIANT/REVIEW_REQUIRED/NON_COMPLIANT`、原因码和证据引用 |
| `ComplianceReviewEvent` | 身份或证据确认、操作者、时间、理由、原 assessment／issue 与生成的新版本；不等同于最终审批 |
| `Recommendation` | `recommendation_id`、`result_id`、推荐／并列候选、合规 assessment、解释与事实引用、`retrieval_id`、制度引用及检索状态、`CURRENT/STALE`；推荐对象须按既定排序通过合规门禁 |
| `PolicySet / PolicyClause` | 不可变集合版本、文档／版本／条款 ID、范围、生效区间、标题、章节、正文、哈希、虚构标识及经审阅的 `control_code`／参数；成员版本显式列入清单 |
...
- 比较结果来自已核验输入和确定性工具；
- 推荐引用的比较结果、制度集合和供应商注册表与当前输入版本一致；
- 推荐候选的合规 assessment 为 `COMPLIANT`，所有更高排名候选均为 `NON_COMPLIANT`；不得绕过仍为 `REVIEW_REQUIRED` 的更高候选；
- 人工审核人明确批准该推荐版本；
...
制度检索使用独立的 12 个问题（8 开发／4 留出），分别评测 BM25、pgvector、融合召回 Recall@10、rerank Recall@3、引用语义支持、无答案、冲突和旧版过滤。供应商身份、批准状态、RoHS 有效期、合规三态及候选选择使用独立确定性矩阵，预定边界必须全部通过。两类结果分开报告，不能用检索指标代替合规正确性。

开发顺序与项目方案保持一致：第一周建立契约、版本骨架和最小部署；第二周完成补问、更新、制度检索、批准供应商／RoHS 门禁、审批及简单报告；最终周执行留出评测、修复和提交。ISO、框架合同、自由文本偏好、开放式多轮补问和额外版式不列为必交付。
