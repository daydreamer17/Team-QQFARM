...
> 文档状态：Final（2026-09-14）。后续实施以无版本后缀的 [WORKFLOW.md](WORKFLOW.md) 为正式入口。
>
...
| RAG 模块 | 用 BM25 基线及可选独立向量库、本地 Embedding／Rerank 检索适用制度，返回可核验引用 | 通过语义搜索猜测批准状态、证书有效期或独立裁决合规 |
...
初步价格与可行性结果
  ...
  → 在对应版本与范围内构造制度查询
  → BM25 Top-3 基线检索
  → 可选：独立向量数据库＋本地 embedding 模型召回 Top-10
  → 可选：本地 rerank 模型融合并重排取 Top-3
...
适用的 control_code 先由制度清单中的结构化范围确定，不能依赖 Top-k 检索结果决定。BM25、向量召回和 Rerank 只负责找到相应原文与解释依据；任何必需控制码没有检索到支持条款时进入 `REVIEW_REQUIRED`，不能少执行一项检查后仍判定合规。平均 Recall 只用于评价检索效果；一次具体采购必须达到全部 required control_code 的逐项证据覆盖，才允许生成 `COMPLIANT`。LLM 和 RAG 不能临时发明控制码、阈值或规则。批准状态、证书编号和有效期由 PostgreSQL 精确查询，不能用相似度分数代替。
...
BM25、embedding、rerank 和解释调用分别记录检索器／模型版本、次数、延迟、用量和错误。独立向量数据库中的索引绑定制度集合版本、条款 ID、内容哈希、embedding 模型与预处理版本，并可从 PostgreSQL 权威条款重建。本地模型或向量库不可用时可以显式降级到 BM25，但必须保存失败与降级轨迹。`NO_EVIDENCE`、`CONFLICT` 和 `ERROR` 必须分别保存并映射为 `REVIEW_REQUIRED`。报价参考答案、评测标签和生成器映射不得进入运行索引或提示词。
...
| 可重建条款向量和近邻索引 | 可选独立向量数据库；绑定 PostgreSQL 中的权威版本和内容哈希 |
...
| 7 | 批准供应商＋有效 RoHS | BM25 及启用的向量／rerank 路径检索到正确制度，全部 required control_code 均有当前版本且支持主张的引用，结构化记录核验通过后发布推荐 |
...
| BM25 基线＋可选独立向量库和本地 Embedding／Rerank 制度检索 | Week2 待实现 |
