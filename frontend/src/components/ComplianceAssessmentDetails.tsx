        ...
        {rows.map((row) => {
          ...
          return <details className="compliance-frozen-supplier" key={row.quote_id}>
            ...
            <div className="compliance-frozen-supplier-body">
              ...
              {evidence.length > 0 && <details className="compliance-frozen-materials"><summary>已提交材料（{evidence.length}）</summary><div>{evidence.map((item) => <article key={item.evidence_id}><strong>{item.facts.material_number} · 第 {item.version} 版</strong>{item.files.map((file) => <a className="compliance-file" key={file.file_id} href={complianceEvidenceUrl(taskId, item.evidence_id, file.file_id)}>{file.original_filename}</a>)}</article>)}</div></details>}
            </div>
          </details>
        })}
      </div>}
      {amountRequirements.length > 0 && <details className="compliance-frozen-amounts"><summary>金额条件与后续动作（{amountRequirements.length}）</summary><div className="compliance-table-scroll"><table className="supplier-data-table"><thead><tr><th>执行阶段</th><th>判断</th><th>后续动作</th></tr></thead><tbody>{amountRequirements.map((item, index) => <tr key={`${item.quote_id}:${item.execution_stage}:${index}`}><td>{executionStageLabel(item.execution_stage)}</td><td>{item.triggered ? '达到条件' : item.triggered === false ? '未达到条件' : '尚待检查'}</td><td>{item.triggered ? item.action ?? '按制度办理' : '—'}</td></tr>)}</tbody></table></div><p>以上为制度要求，不代表采购获批。</p></details>}
