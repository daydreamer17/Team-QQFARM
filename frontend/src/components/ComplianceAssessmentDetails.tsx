...
import { aggregateControlStatus, compactCheckStatusLabel, compactComplianceStatusLabel, complianceCheckHref, complianceReasonLabel, executionStageLabel, groupComplianceChecks } from '../lib/compliance'
...
export function ComplianceAssessmentDetails({ assessment, assessments, taskId, resultId, historical = false, legacy = false }: {
  ...
  return <details className="compliance-frozen-details">
    <summary className="compliance-frozen-summary"><span><strong>Compliance</strong><small>{assessment?.task_revision ? `Revision ${assessment.task_revision} · ` : ''}{rows.length} {rows.length === 1 ? 'supplier' : 'suppliers'}</small></span><span className="compliance-frozen-summary-counts"><b>{compliant} verified</b>{review > 0 && <b>{review} pending</b>}{excluded > 0 && <b>{excluded} excluded</b>}</span></summary>
      ...
      {rows.length === 0 ? <p className="audit-empty">This result has no supplier-level compliance records.</p> : <div className="compliance-frozen-suppliers">
        {rows.map((row) => {
          ...
          return <details className="compliance-frozen-supplier" key={row.quote_id}>
            <summary><span className="compliance-frozen-supplier-name"><strong>{row.supplier_name ?? row.supplier_id ?? row.quote_id}</strong><small>Revision {row.quote_version}</small></span><span><small>Status</small><Badge status={row.status}>{compactComplianceStatusLabel(row.status)}</Badge></span><span><small>Recommendation</small><Badge status={row.eligibility ?? 'UNVERIFIED'}>{compactComplianceStatusLabel(row.eligibility ?? 'UNVERIFIED')}</Badge></span><span className="compliance-frozen-issue-count">{pending > 0 ? `${pending} pending` : 'Passed'}</span></summary>
