    ...
    <div className="compliance-control-grid">
      {groups.map(([control, checks]) => {
        ...
        return <article className={`compliance-control-card compliance-control-${stateTone(status)}`} key={control}>
          ...
          {!readonly && (control !== 'AMOUNT_APPROVAL' || checks.some((check) => check.triggered === true)) && <div className="compliance-control-action"><button className="button button-secondary" type="button" onClick={() => onEdit(control as ComplianceEvidenceFacts['control_code'], currentRecord)}>{currentRecord ? 'Replace' : 'Upload'}</button></div>}
        </article>
      })}
    </div>
  </div>
...
export function CompliancePage() {
  ...
  const start = useMutation({ mutationFn: () => api.startCompliance(taskId, task.data!.task_revision, createIdempotencyKey()), onSuccess: refresh })
  ...
  return <div className="page-stack compliance-page">
    ...
    {suppliers.length > 0 && <section className="card compliance-results-card"><div className="section-heading"><div><h3>Checks</h3></div><span>{suppliers.length} suppliers</span></div>
      <div className="compliance-table-scroll"><table aria-label="Supplier checks" className="supplier-data-table compliance-table"><thead><tr><th>Supplier</th><th>Supplier eligibility</th><th>RoHS</th><th>Amount approval</th><th>Recommendation eligibility</th><th>Evidence and sources</th></tr></thead><tbody>
        {pagination.pageItems.map((supplier) => { const admission = aggregateControlStatus(supplier.checks, 'APPROVED_SUPPLIER'); const rohs = aggregateControlStatus(supplier.checks, 'ROHS_COMPLIANCE'); const amountApproval = aggregateControlStatus(supplier.checks, 'AMOUNT_APPROVAL'); const supplierName = supplier.supplier_name ?? supplier.supplier_id ?? supplier.quote_id; const expanded = supplier.quote_id === linkedQuote || expandedQuotes.includes(supplier.quote_id); const supplierEvidence = data.evidence.filter((record) => record.quote_id === supplier.quote_id); return <Fragment key={supplier.quote_id}><tr id={complianceAnchorId(supplier.quote_id)} tabIndex={-1}><td><strong>{supplierName}</strong><small>Revision {supplier.quote_version}</small></td><td><StateBadge status={admission} label={compactCheckStatusLabel(admission)} /></td><td><StateBadge status={rohs} label={compactCheckStatusLabel(rohs)} /></td><td><StateBadge status={amountApproval} label={compactCheckStatusLabel(amountApproval)} /></td><td><StateBadge status={supplier.eligibility ?? 'UNVERIFIED'} label={compactComplianceStatusLabel(supplier.eligibility ?? 'UNVERIFIED')} /></td><td>
