...
function SupplierCheckDetails({ supplier, clauses, evidence, taskId, linkedCheck, readonly, onReplace }: {
  ...
  readonly: boolean
  onReplace: (record: ComplianceEvidenceRecord) => void
}) {
...
function Confirmation({ workspace, onRefresh }: { workspace: ComplianceWorkspace; onRefresh: () => Promise<void> }) {
  ...
  return <section className="card compliance-confirmation"><div className="section-heading"><div><p className="eyebrow">FINAL CHECK</p><h3>确认本次处理结果</h3><p>未补齐材料的供应商保持“未核验”；确认仅表示接受当前缺项，不代表采购批准。</p></div><span>{groupedMissing.length} 类待处理</span></div>
    ...
    <div className="compliance-confirm-actions"><p>{missing.length > 0 ? `确认后仍保留 ${missing.length} 个原始检查项，系统不会把它们视为通过。` : '当前没有需要明确保留的缺项。'}</p><button className="button button-submit" type="button" disabled={disabled || confirm.isPending} onClick={() => confirm.mutate()}>{confirm.isPending ? '正在确认…' : '确认处理结果并进入决策'}</button></div>
...
export function CompliancePage() {
  ...
  const policyTitle = [...new Set(clauses.map((clause) => clause.title).filter(Boolean))].join('、')
  ...
  const policySummary = data.policy_binding
    ? [policyTitle || '已绑定制度', data.policy_binding.policy_set_version && `v${data.policy_binding.policy_set_version}`, policyCategory, policyRegion].filter(Boolean).join(' · ')
    : '本任务未启用制度检查'
  return <div className="page-stack compliance-page">
    ...
    <section className="card compliance-context">
      <div className="compliance-context-heading"><h2>制度检查</h2><p>{policySummary}</p><p className="compliance-scope-copy">核对供应商材料与金额条件，再进入决策比较</p></div>
    ...
    {!data.assessment && data.stage.status !== 'PROCESSING' && <section className="card"><h3>等待报价处理完成</h3><p>这里仅展示当前版本的检查，历史结果不会作为当前制度结论显示。</p><Link to={`/tasks/${taskId}/quotes/new`}>查看报价与审核</Link></section>}
    ...
    {suppliers.length > 0 && <section className="card compliance-results-card"><div className="section-heading"><div><p className="eyebrow">SUPPLIER CHECKS</p><h3>供应商检查结果</h3><p>已核验候选优先；明确不符合的候选不参与推荐。</p></div><span>{suppliers.length} 家供应商</span></div>
      <div className="compliance-table-scroll"><table aria-label="供应商检查结果" className="supplier-data-table compliance-table"><thead><tr><th>供应商</th><th>供应商准入</th><th>RoHS</th><th>制度结论</th><th>推荐资格</th><th>材料与依据</th></tr></thead><tbody>
        ...
        </td></tr>{expanded && <tr className="compliance-detail-row"><td colSpan={6}><SupplierCheckDetails supplier={supplier} clauses={clauses} evidence={supplierEvidence} taskId={taskId} linkedCheck={linkedCheck} readonly={readonly} onReplace={(record) => setTarget({ supplier, control: record.control_code, record, revision: data.task_revision })} /></td></tr>}</Fragment>})}
    ...
    {amountRequirements.length > 0 && <section className="card compliance-amount-card"><div className="section-heading"><div><p className="eyebrow">AMOUNT RULES</p><h3>金额条件与后续动作</h3><p>金额规则与供应商资格分开显示，不代表采购已经获批。</p></div><span>{amountRequirements.length} 条条件</span></div><div className="compliance-table-scroll"><table aria-label="金额条件与后续动作" className="supplier-data-table compliance-amount-table"><thead><tr><th>供应商</th><th>执行阶段</th><th>当前金额 / 门槛</th><th>判断</th><th>后续动作</th></tr></thead><tbody>{amountRequirements.map((item, index) => {
      const supplierName = item.supplier_name ?? suppliers.find((supplier) => supplier.quote_id === item.quote_id)?.supplier_name ?? '供应商身份待核对'
      const deferred = item.execution_stage === 'AFTER_SELECTION'
      const outcome = item.triggered === true ? (deferred ? '选择后将触发' : '已触发') : item.triggered === false ? '未触发' : '待检查'
      return <tr key={`${item.quote_id}:${item.execution_stage}:${item.threshold}:${item.action}:${index}`}><td><strong>{supplierName}</strong></td><td>{executionStageLabel(item.execution_stage)}</td><td>{item.amount && item.currency ? `${item.currency} ${item.amount}` : '金额待确认'}{item.threshold && <small>门槛：{item.currency ?? ''} {item.threshold}</small>}</td><td><StateBadge status={item.triggered === true ? 'REVIEW_REQUIRED' : item.triggered === false ? 'PASS' : 'NOT_EVALUATED'} label={outcome} /></td><td>{item.triggered ? item.action ?? '按制度办理' : '无需新增动作'}</td></tr>
