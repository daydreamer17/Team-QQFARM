...
def create_review_event(
    ...
    *,
    ...
    """Create a version-bound confirmation for an unchanged candidate field."""
    ...
    if candidate.validation_status not in expected_statuses:
    ...
    if action in {
        HumanReviewAction.CONFIRM_VALUE,
        HumanReviewAction.CONFIRM_CONFLICT,
    ...
    selected_reason = reason_code or {
        HumanReviewAction.CONFIRM_VALUE: "HUMAN_CONFIRMED_VALUE",
        HumanReviewAction.CONFIRM_MISSING: "DOCUMENT_DOES_NOT_STATE_VALUE",
        HumanReviewAction.CONFIRM_CONFLICT: "DOCUMENT_CONFLICT_UNRESOLVED",
    }[action]
    identity = {
        "action": action.value,
    ...
    review_event_id = stable_id("review_event", identity)
    return ReviewEvent(
